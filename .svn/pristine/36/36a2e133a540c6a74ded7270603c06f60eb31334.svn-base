						...
						<s:iterator var="j" begin="0" end="%{#cells-1}" step="1">
							...
							<td width='<s:property value="100/#cells" />%'>
								<s:if test="#idx < surveys.size">
									<s:a action="enterSurvey?survey.id=%{surveys[#idx].id}" namespace="/engageSurvey" cssClass="aList">
										<img alt="<s:property value='surveys[#idx].title' />" src="<s:url value="%{surveys[#idx].logoPhotoPath}"/>" height="80px" width="80px" 
										onerror="javascript:this.src='<s:url value="/upload/default.jpg"/>'"><br>
