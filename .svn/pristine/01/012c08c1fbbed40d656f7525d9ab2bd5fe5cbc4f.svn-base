...
<html>
	...
	<body>
		...
		<table>
			...
			<tr>
				<td colspan="10" class="tdPHeaderR"><s:a action="addRole" namespace="/role">添加角色</s:a></td>
			</tr>
			<tr>
				<td colspan="10" style="height: 5px"></td>
			</tr>
				...
				<tbody>
					<s:iterator value="allRoles" status="st">
						...
						<tr>
							...
							<td><s:a action="editRole?role.id=%{#roleId}" namespace="/role" cssClass="aList">修改</s:a></td>
							<td><s:a action="deleteRole?role.id=%{#roleId}" cssClass="aList">删除</s:a></td>
						</tr>
					</s:iterator>
				</tbody>
			</table>
