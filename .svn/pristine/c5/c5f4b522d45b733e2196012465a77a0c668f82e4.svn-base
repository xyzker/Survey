				...
				<tbody>
					<s:iterator value="allUsers" status="st">
						...
						<tr>
							...
							<td><s:a action="editAuthorize?user.id=%{#userId}" namespace="/authorize" cssClass="aList">修改授权</s:a></td>
							<td><s:a action="clearAuthorize?user.id=%{#userId}" namespace="/authorize" cssClass="aList">清除授权</s:a></td>
						</tr>
					</s:iterator>
				</tbody>
			</table>
