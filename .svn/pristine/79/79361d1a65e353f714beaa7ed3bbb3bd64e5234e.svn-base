...
<html>
	...
	<body>
		...
		<s:form action="doSurvey" namespace="/engageSurvey" method="post" >
			<s:hidden name="currentPage.id" />
			<table>
				<tr>
				...
				<tr>
					<!-- 页面标题 -->
					<td colspan="2" class="tdPHeaderL"><s:property value="currentPage.title" /></td>
				</tr>
				<tr>
					<td width="30px"></td>
					<td>
						<table>
							<!-- 遍历问题集合 -->
							<s:iterator var="q" value="currentPage.questions">
								<!-- 设置变量,对问题的id进行保持 -->
				...
				<tr>
					<td colspan="2" align="center">
						<!-- 构造上一步按钮 -->
						<s:if test="currentPage.orderno != #session.current_survey.minOrderno">
							<input type="submit" name='submit_pre' value='<s:property value="#session.current_survey.preText"/>' class="btn">
						</s:if>
						<!-- 构造下一步按钮 -->
						<s:if test="currentPage.orderno != #session.current_survey.maxOrderno">
